...
test("all restricted Client layers belong to the layers permission", () => {
  ...
  expect(parseClientPermissionDeclarations({ layers: ["wallpaper"] })).toEqual({ layers: ["wallpaper"] })
...
test("layer permissions share the declaration and stored permission contracts", () => {
  ...
  const declaration: ClientPermissionDeclarations = { layers: ["under", "over", "wallpaper"] }
  ...
  expect(parsePermission("layers", ["over", "wallpaper", "under", "over"])).toEqual(["over", "wallpaper", "under"])
