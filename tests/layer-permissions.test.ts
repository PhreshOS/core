...
test("all restricted Client layers belong to the layers permission", () => {
  ...
  expect(parseProgramPermissionDeclarations({ layers: ["wallpaper", "start-menu"] })).toEqual({ layers: ["wallpaper", "start-menu"] })
...
test("layer permissions share the declaration and stored permission contracts", () => {
  ...
  const declaration: ProgramPermissionDeclarations = { layers: ["under", "over", "wallpaper", "start-menu"] }
